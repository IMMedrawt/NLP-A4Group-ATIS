...
def main():
    ...
    output_dir = f'../outputs/{dataset_name}_linear_{split_type}'
    ...
    if not X_train_filtered:
        raise RuntimeError("[FATAL] No training examples available after filtering.")
    ...
    clf.train(X_train_vec, y_train_filtered)
    ...
    print(f"[Linear Classifier] Accuracy on {split_type} (filtered classes): {acc:.4f}")
